...
O Grafana já vem com o datasource e um dashboard provisionados, prontos no primeiro acesso.
...
```
   ...
   postgres_exporter ┘   (coleta e         (dashboards)
                          armazena)
```

O Prometheus faz scrape dos exporters a cada 15 segundos e guarda 15 dias de histórico. O Grafana lê o Prometheus e desenha os painéis.

## 🛠️ Componentes

| Serviço | Imagem | Porta | Função |
|---------|--------|-------|--------|
| Prometheus | prom/prometheus | 9090 | Coleta e armazena as métricas |
...
Só o Grafana (3000) e o Prometheus (9090) são expostos ao host. Os exporters ficam na rede interna, acessíveis apenas pelo Prometheus.
...
| Serviço | URL | Login |
|---------|-----|-------|
| Grafana | http://192.168.100.3:3000 | admin e a senha gerada |
| Prometheus | http://192.168.100.3:9090 | sem login |

No Grafana, o dashboard "Homelab Overview" já aparece na lista, com as três camadas. No Prometheus, a página Status > Targets mostra os exporters e se estão sendo coletados (todos devem estar "UP").
...
└── grafana/
    └── provisioning/
        ...
        └── dashboards/
            ...
            └── homelab-overview.json # Dashboard das três camadas
```
